...
var _firebaseInitializer = require('../../the_conch/firebaseInitializer');
...
var Dashboard = function (_Component) {
  ...
  _createClass(Dashboard, [{
    ...
    value: function componentWillMount() {
      ...
      var actionsRef = _firebaseInitializer2.default.database().ref('actions');
      actionsRef.on('child_added', function (snapshot) {
        var action = { name: snapshot.val(), command: snapshot.val(), id: snapshot.key };
        _this2.setState({ actions: [action].concat(_this2.state.actions) });
      });
    }
  }, {
    key: 'handleClick',
